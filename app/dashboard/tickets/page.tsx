...
export default function TicketsPage() {
  ...
  useEffect(() => {
    const fetchReferenceData = async () => {
      try {
        const [usersRes, teamsRes, categoriesRes] = await Promise.all([
          fetch('/api/users'),
          fetch('/api/teams'),
          fetch('/api/categories')
        ]);
        
        const [usersData, teamsData, categoriesData] = await Promise.all([
          usersRes.json(),
          teamsRes.json(),
          categoriesRes.json()
        ]);
  ...
  const sortedTickets = [...tickets].sort((a, b) => {
    ...
    if (key === 'requester' && typeof aVal === 'object') {
      aVal = aVal?.full_name || '';
      bVal = bVal?.full_name || '';
    }
            ...
            <div className="p-6 border-b border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <button
                    onClick={() => handleUserClick(selectedTicket.requester.id)}
                ...
                <div>
                  ...
                  <span className="ml-2 text-gray-600">
                    {selectedTicket.team?.name || 'Unknown'}
                  </span>
                </div>
                <div>
                  <label className="font-medium text-gray-700">Category:</label>
                  <span className="ml-2 text-gray-600">
                    {selectedTicket.category?.name || 'Unknown'}
                  </span>
                </div>
                <div>
